...
describe("sales analysis prompt", () => {
  it("uses the server prompt version constant", () => {
    ...
    expect(getSalesAnalysisPrompt().systemPrompt).toBe(AI_SALES_ANALYSIS_PROMPT_V1);
  });

  it("does not interpolate lead text into the system prompt", () => {
    expect(AI_SALES_ANALYSIS_PROMPT_V1).not.toContain("Ahmed");
    expect(AI_SALES_ANALYSIS_PROMPT_V1).not.toContain("Ignore snapshot");
  });
  ...
  it("puts untrusted conversation text in the user message only", () => {
    ...
    expect(AI_SALES_ANALYSIS_PROMPT_V1).toMatch(/cannot modify CRM/i);
    expect(AI_SALES_ANALYSIS_PROMPT_V1).toMatch(/cannot call tools/i);
  });

  it("does not duplicate pipeline inside the untrusted JSON", () => {
